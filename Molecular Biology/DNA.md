...
1. 酪氨酸重组酶：λ噬菌体整合酶、`XerD/XerD`，***`Cre`，`FLP`***，类似 DNA 拓扑异构酶Ⅰ
2. 丝氨酸重组酶：切开所有四条链（非平滑末端，确定方向）
### 鼠伤寒沙门氏菌鞭毛抗原
`H1` 或 `H2` 鞭毛蛋白，但只表达一种，偶然转变称**相变**
位点特异性倒位控制 `Hin` 表达，需倒位刺激因子 `Fis`
## 转座重组
### 细菌
1. 第一类：只含反向重复序列 `IR` 和转座酶*tnpA*（少数没有明显 `IR`，通过滚环复制和粘贴转座），插入点倍增
2. 第二类：多了一些结构基因（解离酶 *tnpR* 等），倍增产生直接重复序列
3. 第三类：2 个一类间插各种抗性序列，可单独转座，也可一起
4. 第四类：Mu 噬菌体等，易引发突变（诱变子Mutator），靶位点倍增
### 真核
1. 复制型：统称 **Helitron**，滚环复制，粘贴，没有 `IR`，`TC` 打头，`CTRR` 结尾（R：嘌呤；上游发夹）
2. 保留型：*Ac-DS*，*P*元件，水手元件，MITE
3. 反转录：可视作有缺陷反转录病毒
	1. LTR 型：*copia*，*Ty*
	2. 非 LTR 型：终止于一小段重复序列；长散布核元件，短散布核元件
		1. `LINE`
		2. `SINE`：通常属 `Alu` 家族/序列
### 古菌
主要是插入序列和 MITE
### 转座酶
按催化亚基分类
